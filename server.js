...
app.patch('/api/leads/:id', authMiddleware, (req, res) => {
  ...
  db.prepare('UPDATE leads SET status = ? WHERE id = ? AND user_id = ?').run(status, req.params.id, req.user.id);
  res.json({ ok: true });
});
...
app.post('/api/chat/message', authMiddleware, async (req, res) => {
  ...
  // Базовый системный промпт + настройки пользователя
...
Твоя задача:
1. Отвечать клиентам естественно и тепло на русском языке
2. Узнавать их потребность
3. Записывать на встречу/консультацию когда уместно
4. Если клиент задаёт сложный вопрос — предлагать связаться с владельцем

Будь кратким, дружелюбным и профессиональным.`;

  // Достаём историю переписки если есть leadId
  let history = [];
  if (leadId) {
  ...
  try {
    ...
    // Сохраняем в БД если есть leadId
    if (leadId) {
...
app.get('/api/chat/history/:leadId', authMiddleware, (req, res) => {
  const messages = db.prepare('SELECT role, content, created_at FROM messages WHERE lead_id = ? ORDER BY created_at ASC').all(req.params.leadId);
  res.json(messages);
});

// ============ APPOINTMENTS ============
app.get('/api/appointments', authMiddleware, (req, res) => {
  const items = db.prepare('SELECT * FROM appointments WHERE user_id = ? ORDER BY scheduled_at ASC').all(req.user.id);
  res.json(items);
});
